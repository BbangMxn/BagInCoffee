...
초기에는 **SvelteKit 기반으로 웹과 앱 경험을 함께 가져가려는 방향**으로 시작했지만, 실제 개발을 진행하면서 **앱 개발 관점의 한계**가 분명했습니다. 그래서 핵심 사용자 경험은 **Flutter로 전환**했고, 동시에 장비 영역은 **BagInDB라는 별도 프로젝트**로 분리했습니다. 최종적으로는 **Supabase가 인증/커뮤니티를 담당하고, BagInDB가 장비 데이터를 담당하는 구조**로 시스템을 엮었습니다.
...
### 2. 장비 영역을 BagInDB라는 별도 프로젝트로 분리했다

- 사용자, 인증, 커뮤니티 성격의 데이터는 Supabase 축에서 처리했습니다.
- 브랜드, 카테고리, 제품 스펙처럼 성격이 다른 장비 영역은 **BagInDB라는 별도 프로젝트**로 분리했습니다.
...
### Phase 3. 장비 영역을 BagInDB라는 별도 프로젝트로 분리

- 장비/브랜드/카테고리 API를 별도 Rust 서비스인 BagInDB로 구성
...
- **별도 프로젝트 분리**
  인증/커뮤니티 영역은 Supabase 축으로 두고, 장비 데이터 영역은 BagInDB라는 별도 프로젝트로 분리해 시스템을 구성했습니다.
...
| `BagInDB` | 장비 데이터를 담당하는 별도 프로젝트 | Rust, Axum, SQLx, Redis |
