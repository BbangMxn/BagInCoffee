...
개발 과정에서 두 가지 문제가 분명해졌습니다. 첫째, **Svelte 기반 접근으로는 앱 수준 사용자 경험을 만들기 어렵다**는 점이었고, 둘째, **장비 데이터 조회 비용과 응답 부담을 더 줄일 필요가 있다**는 점이었습니다. 이를 해결하기 위해 핵심 사용자 경험은 **Flutter로 전환**했고, 장비 영역은 **도메인 기반의 별도 프로젝트 `BagInDB`**로 분리한 뒤 **Redis 캐시**를 도입했습니다. 최종적으로는 **Supabase가 인증/커뮤니티를 담당하고, BagInDB가 장비 데이터를 담당하는 구조**로 시스템을 엮었습니다.
