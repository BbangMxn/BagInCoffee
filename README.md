...
- 장비 데이터 계층의 리소스 사용을 최대한 줄이기 위해 백엔드는 Rust를 선택
...
- **Rust 선택 이유**
  장비 데이터 API는 반복 조회가 많고 오래 켜둘 가능성이 높다고 봤기 때문에, 백엔드 리소스 사용을 최대한 줄이기 위해 Rust를 선택했습니다.
...
- SvelteKit으로 웹 중심 프로토타입을 먼저 구성
- Supabase를 인증/커뮤니티 축으로 사용

```mermaid
graph TB
    A[SvelteKit Web Prototype]
    B[Supabase\nAuth + Community Data]
    C[Cloudflare R2]
...
- 웹앱 방식의 한계를 느끼고 Flutter로 웹/모바일 클라이언트를 재구성
- 클라이언트 경험을 하나의 프레임워크로 통합
...
- 장비 도메인을 BagInDB라는 별도 프로젝트로 분리
- 반복 조회가 많은 장비 데이터는 Redis 캐시로 최적화
...
| `BagInDB` | 장비 도메인을 분리한 저리소스 백엔드 프로젝트 | Rust, Axum, SQLx, Redis |
