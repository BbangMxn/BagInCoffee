...
개발 과정에서 두 가지 문제가 분명해졌습니다. 첫째, **Svelte 기반 접근으로는 앱 수준 사용자 경험과 크로스플랫폼 일관성을 만들기 어렵다**는 점이었고, 둘째, **장비 데이터 조회 비용과 응답 부담을 더 줄일 필요가 있다**는 점이었습니다. 이를 해결하기 위해 클라이언트는 **Flutter로 재구성해 웹과 모바일을 모두 전환**했고, 장비 영역은 **도메인 기반의 별도 프로젝트 `BagInDB`**로 분리한 뒤 **Redis 캐시**를 도입했습니다. 최종적으로는 **Flutter(Web + Mobile) + Supabase + BagInDB** 구조로 시스템을 엮었습니다.
...
- 하지만 앱 수준의 사용자 경험과 웹/모바일 일관성을 만들기에는 구조적 한계가 있다고 판단했습니다.
...
- **SvelteKit -> Flutter 전환 판단**
  Svelte 기반 접근은 빠른 검증에는 유리했지만, 앱 개발과 크로스플랫폼 운영 관점에서는 한계가 있었습니다. 그래서 핵심 클라이언트를 Flutter로 전환해 웹과 모바일을 함께 가져갔습니다.
