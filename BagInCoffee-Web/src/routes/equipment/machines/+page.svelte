...
<div class="max-w-6xl mx-auto px-4 py-6">
	<!-- Header -->
	<div class="mb-8">
		<h1 class="text-3xl font-bold text-[#5d4a3f] mb-2">에스프레소 머신</h1>
		<p class="text-[#7f6251]">프리미엄 에스프레소 머신을 비교하고 선택하세요</p>
	</div>
		...
		{#each machines as machine}
			...
			>
				...
				<div class="aspect-[4/3] bg-[#f5f5f0] relative overflow-hidden">
					...
					>
						<div class="text-[#bfa094]/30 text-6xl font-light">{machine.brand.slice(0, 1)}</div>
					</div>
					<div
				...
				<div class="p-5">
					...
					<div class="flex items-center justify-between mb-2">
						...
						<div class="flex items-center gap-1">
							...
							<span class="text-sm text-[#5d4a3f] font-medium">{machine.rating}</span>
						</div>
					</div>
					...
					<!-- Description -->
					<p class="text-sm text-[#7f6251] mb-4 line-clamp-2">{machine.description}</p>
						...
						<div>
							{#if machine.price}
								<span class="text-xl font-bold text-[#bfa094]"
									>{machine.price.toLocaleString()}원</span
								>
							{:else if machine.price_range}
								<span class="text-xl font-bold text-[#bfa094]">{machine.price_range}</span>
							{:else}
								<span class="text-sm text-[#9ca3af]">가격 문의</span>
							{/if}
						</div>
						<span class="text-xs text-[#9ca3af]">리뷰 {machine.reviews_count || 0}</span>
					</div>
				</div>
			</a>
		{/each}
